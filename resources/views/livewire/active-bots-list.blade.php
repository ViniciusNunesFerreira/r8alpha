<div wire:poll.15s="loadBots" class="glass-effect p-6 rounded-xl">
    ...
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-4">
        ...
        <!--  route('bots.index')  -->
        <a href="{{ route('bots.index') }}" class="px-4 py-2 bg-primary hover:bg-primary/80 rounded-lg font-semibold transition whitespace-nowrap">
            View All
        </a>
    </div>
        ...
        <div class="p-4 bg-white/5 hover:bg-white/10 rounded-xl transition border border-white/10 hover:border-primary/30">
            <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-4 gap-4">
                <div class="flex items-center space-x-4 w-full sm:w-auto">
                    ...
                    <div class="min-w-0 flex-1">
                        <div class="flex items-center space-x-2 mb-1 flex-wrap">
                            <h4 class="font-bold text-lg truncate">{{ $bot->instance_id }}</h4>
                            <span class="px-2 py-1 text-xs rounded-full {{ $bot->is_active ? 'bg-success/20 text-success' : 'bg-gray-700 text-gray-400' }} flex-shrink-0">
                                {{ $bot->is_active ? 'Active' : 'Inactive' }}
                            </span>
                        </div>
                        <div class="flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-gray-400">
                            <span class="truncate">Plan: {{ $bot->investment->investmentPlan->name }}</span>
                            <span class="hidden sm:inline">•</span>
                            <span class="truncate">Investment: ${{ number_format($bot->investment->amount, 2) }}</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex items-center space-x-2 flex-shrink-0">
                    <button 
                        ...
                        class="px-3 sm:px-4 py-2 rounded-lg font-semibold text-xs sm:text-sm transition {{ $bot->is_active ? 'bg-red-500/20 text-red-500 hover:bg-red-500/30' : 'bg-success/20 text-success hover:bg-success/30' }}">
                        {{ $bot->is_active ? 'Stop' : 'Start' }}
                    </button>
                    <!--  route('bots.show', $bot)  -->
                    <a href="{{ route('bots.show', $bot) }}" class="p-2 bg-white/5 hover:bg-white/10 rounded-lg transition">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3 sm:gap-4">
                ...
                <div class="text-center p-3 bg-white/5 rounded-lg">
                    ...
                    <p class="text-base sm:text-lg font-bold text-success truncate">+${{ number_format($bot->total_profit, 2) }}</p>
                </div>

                <!-- Current Balance -->
                <div class="text-center p-3 bg-white/5 rounded-lg">
                    <p class="text-xs text-gray-400 mb-1">Balance</p>
                    <p class="text-base sm:text-lg font-bold text-white truncate">${{ number_format($bot->investment->current_balance, 2) }}</p>
                </div>

                <!-- Total Trades -->
                <div class="text-center p-3 bg-white/5 rounded-lg">
                    <p class="text-xs text-gray-400 mb-1">Total Trades</p>
                    <p class="text-base sm:text-lg font-bold text-white">{{ $bot->total_trades }}</p>
                </div>
                ...
                <div class="text-center p-3 bg-white/5 rounded-lg">
                    ...
                    <div class="flex items-center justify-center">
                        <p class="text-base sm:text-lg font-bold text-white mr-1">{{ number_format($bot->success_rate, 1) }}%</p>
                ...
                <!-- Last Trade -->
                <div class="text-center p-3 bg-white/5 rounded-lg col-span-2 sm:col-span-1">
